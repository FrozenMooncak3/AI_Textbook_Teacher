...
test('task 10 OCR deployment files expose configurable OCR host and port', async () => {
  ...
  assert.match(ocrServerSource, /os\.environ\.get\("OCR_PORT", sys\.argv\[1\] if len\(sys\.argv\) > 1 else "8000"\)/)
  assert.match(screenshotOcrSource, /const OCR_SERVER_HOST = process\.env\.OCR_SERVER_HOST \|\| '127\.0\.0\.1'/)
  assert.match(screenshotOcrSource, /const OCR_SERVER_PORT = Number\(process\.env\.OCR_SERVER_PORT\) \|\| 9876/)
