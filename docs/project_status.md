...
**进行中**：**M4.7 OCR + KP 成本架构 ⏳ Plan ready，等执行**（2026-04-26 brainstorm 完结 + plan reviewer 通过）。M4.6 OCR 管线修复 ✅ 代码全修完（T15+T16+T17 实测通过），但 book 18 暴露 KP 提取阶段 Google AI Studio 余额耗尽 + 单本成本 8-15 元 → 触发 M4.7 战略级重设
...
**下一步**：执行 M4.7 plan。两条路径供选择：
1. **Subagent-Driven（推荐）**：每 task fresh subagent 派给 Codex/Gemini，Phase 0-2 后端基础（schema + provider + 服务层 + API）→ Phase 3 PPT 解析（Python）→ Phase 4 前端 → Phase 5 env + 回归测试 → Phase 6 收尾 + 关 brainstorm
2. **Inline Execution**：在当前 session 用 task-execution skill 直接 dispatch，批量 checkpoint review

⚠️ **执行前置**：Phase 5 env 切换需用户提供 3 个 secret —— `DEEPSEEK_API_KEY`（platform.deepseek.com 注册）+ `DASHSCOPE_API_KEY`（阿里云 DashScope）+ `RESEND_API_KEY`（resend.com / 可选，无则降级 console.warn）。Phase 0-4 不依赖以上 secret 可先行落地
