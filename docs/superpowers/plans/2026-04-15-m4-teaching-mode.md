...
| 项 | 值 |
|------|------|
| docker compose 服务 | `app`、`db`、`ocr` |
| DB 连接 | `postgresql://dev:dev@db:5432/textbook_teacher`（**非 `postgres/postgres/textbook`**；psql 命令用 `-U dev -d textbook_teacher`） |
| 在容器内跑脚本 | `docker compose exec app npx tsx <path>.ts`（repo 既有脚本 `scripts/test-prompt-templates.ts` 即此模式） |
| 在宿主机跑脚本 | `DATABASE_URL=postgresql://dev:dev@localhost:5432/textbook_teacher npx tsx <path>.ts`（需要 5432 端口映射；当前 `docker-compose.yml` 未暴露 db 5432 对外——测试全部以 `docker compose exec app` 方式跑） |
| `gen_random_uuid()` 前提 | 必须 `CREATE EXTENSION IF NOT EXISTS pgcrypto`。当前 `schema.sql` **没有这行**，Task 3 会加。 |
...
| 路径 | 改动 |
|------|-----|
| `src/lib/schema.sql` | 顶部加 `CREATE EXTENSION IF NOT EXISTS pgcrypto;` + knowledge_points `type` CHECK 替换（命名约束 + `DROP CONSTRAINT IF EXISTS` 幂等）+ 新增 `source_anchor JSONB` 列 + `prompt_templates` 新增 `model` 列 + `teaching_sessions` / `user_subscriptions` 新建 + transcript 默认值为完整信封对象 |
...
## Task 0: Docker 环境摸底（pre-flight）

**Why first:** 后续 task 所有 `docker compose exec app ...`、`docker compose exec db psql -U dev -d textbook_teacher ...` 命令都依赖容器就绪。先验一遍能节约每个 task 开头的试错成本。

**Files:**
- Read-only：`docker-compose.yml`、`package.json`、`src/lib/schema.sql`
...
Expected: `Creating/Recreating app / db / ocr` 无 error；`docker compose ps` 三个 service 都 `Up`。

- [ ] **Step 0.2: DB 连通性 smoke**

```bash
docker compose exec db psql -U dev -d textbook_teacher -c "SELECT version();"
```

Expected: 返回一行 PostgreSQL 16 版本字符串。若认证错 → 本 plan 的 DB 连接事实（§Codebase §3）已错，停下来汇报 Claude。

- [ ] **Step 0.3: app 容器 tsx 可用性**

```bash
docker compose exec app npx tsx --version
```

Expected: 版本号输出。若 `tsx` 不在 devDependencies，需补装（`npm install -D tsx`）并 commit 单独一笔。

- [ ] **Step 0.4: 既有 test-prompt-templates 基线**

```bash
docker compose exec app npx tsx scripts/test-prompt-templates.ts
```

Expected: `All prompt template tests passed`。若既有测试本身就红，先排查，再进 Task 1（确保不是本 plan 起始就倒退）。

- [ ] **Step 0.5: 汇报（不 commit，无代码改动）**

此 task 零 commit，输出容器状态截图/粘贴到完成报告即可。
...
- [ ] **Step 1.1: 核实 DB 无 KP 数据（反向确认）**

```bash
docker compose exec db psql -U dev -d textbook_teacher -c "SELECT COUNT(*) FROM knowledge_points;"
```

Expected: `count` 列为 `0`。若非 0，停下来汇报给 Claude（需补数据迁移方案）。
...
替换为**未命名 inline CHECK 去除**（只保留类型声明）：

```sql
type TEXT NOT NULL,
```

然后在 `CREATE TABLE knowledge_points (...);` **之后**追加：

```sql
-- 幂等迁移：旧 CHECK 约束可能有系统生成名 或 新 CHECK 名，两者都 drop
...
- [ ] **Step 1.6: 重建容器让 schema 生效 + seed UPSERT extractor 新模板**

```bash
docker compose down
docker compose up --build -d
docker compose logs app --tail 50 | grep -Ei "error|fatal"
```

Expected: 无 error / fatal。initDb 会自动跑 schema.sql，并在数据非空时对 extractor / coach / examiner / reviewer / assistant 做 UPSERT（seedTemplates 内部逻辑）——extractor 新文本因此被推到 DB。

验证 extractor 模板已更新：

```bash
docker compose exec db psql -U dev -d textbook_teacher -c "SELECT role, stage, LEFT(template_text, 200) FROM prompt_templates WHERE role='extractor' AND template_text LIKE '%position%';"
```
...
```bash
docker compose exec db psql -U dev -d textbook_teacher -c "INSERT INTO knowledge_points (module_id, kp_code, section_name, description, importance, type, detailed_content) VALUES (1, 'T1', 'test-section', 'test-desc', 3, 'position', 'test-content') RETURNING id;"
```

Expected: 失败，错误信息含 `violates check constraint "knowledge_points_type_check"`。若成功插入则约束没生效，停下来排查。

验证 `source_anchor` 列：

```bash
docker compose exec db psql -U dev -d textbook_teacher -c "SELECT column_name FROM information_schema.columns WHERE table_name='knowledge_points' AND column_name='source_anchor';"
```

Expected: 返回一行 `source_anchor`。
...
```bash
docker compose exec app npx tsx scripts/test-m4-task1-kp-migration.ts
```
...
```bash
docker compose exec app npm install zod@^3.23.0
```
...
```bash
docker compose exec app node -e "const { z } = require('zod'); const s = z.object({ n: z.number() }); console.log('zod ok:', s.parse({ n: 1 }).n === 1)"
```
...
```bash
docker compose exec app npx tsx /tmp/zod-ai-smoke.ts && rm /tmp/zod-ai-smoke.ts
```
...
## Task 3: schema 追加（pgcrypto + prompt_templates.model + teaching_sessions + user_subscriptions）
...
- [ ] **Step 3.5: 重建容器应用新 schema**

```bash
docker compose down
docker compose up --build -d
docker compose logs app --tail 30 | grep -Ei "error|fatal"
```

Expected: 无 error / fatal。
...
```bash
docker compose exec app npx tsx scripts/test-m4-task3-schema.ts
```
...
```bash
docker compose exec app npx tsx scripts/test-m4-task4-retry.ts
```
...
- [ ] **Step 7.4: 重新跑 initDb / seedTemplates（通过容器重启自动触发）**
...
```bash
docker compose exec db psql -U dev -d textbook_teacher -c "SELECT role, stage, model FROM prompt_templates WHERE role IN ('extractor','coach','examiner','reviewer','assistant') ORDER BY role, stage;"
```
...
- [ ] **Step 9.3: 重启容器触发 initDb / seedTemplates**
...
```bash
docker compose exec db psql -U dev -d textbook_teacher -c "SELECT stage, LENGTH(template_text) AS len, model FROM prompt_templates WHERE role='teacher' ORDER BY stage;"
```

Expected: 5 行，每行 `model` 为 NULL，`len` > 300。

验证每条含占位符（Task 10 会依赖）：

```bash
docker compose exec db psql -U dev -d textbook_teacher -c "SELECT stage FROM prompt_templates WHERE role='teacher' AND (template_text LIKE '%{kp_content}%' AND template_text LIKE '%{cluster_kps}%' AND template_text LIKE '%{struggling_streak}%');"
```

Expected: 5 行（所有 stage 都含 3 个占位符）。
...
- params 类型报错 → 确认用的是 `params: Promise<{sessionId:string}>` + `await params`（本仓 Next.js 16.1.6，所有 dynamic `[id]/route.ts` 都是 async params，参考 `src/app/api/conversations/[conversationId]/messages/route.ts:22-24`）

- [ ] **Step 10.4: 启容器跑一次手动 smoke**

```bash
docker compose up -d
```

手动通过浏览器登录拿 cookie，用 `curl` 或 REST client：

```bash
# 1. 创建 session
curl -X POST http://localhost:3000/api/teaching-sessions \
  -H "Content-Type: application/json" \
  -b "textbook_teacher_session=<cookie值>" \
  -d '{"moduleId": <有效id>, "clusterId": <有效id>, "depth": "full"}'
# 期望：200，返回 { sessionId, transcript }

# 2. 发消息（sessionId 从上面拿）
curl -X POST "http://localhost:3000/api/teaching-sessions/<sessionId>/messages" \
  -H "Content-Type: application/json" \
  -b "textbook_teacher_session=<cookie值>" \
...
test('teaching session happy path', async (t) => {
  if (!AUTH_COOKIE) {
    t.skip('需要 TEST_AUTH_COOKIE 环境变量（格式：textbook_teacher_session=xxx）')
    return
  }
...
```bash
TEST_AUTH_COOKIE="textbook_teacher_session=<cookie>" TEST_MODULE_ID=<id> TEST_CLUSTER_ID=<id> \
  docker compose exec -e TEST_AUTH_COOKIE -e TEST_MODULE_ID -e TEST_CLUSTER_ID \
  app npx tsx scripts/test-m4-task10-messages-api.ts
```
...
- [ ] `docker compose exec app npx tsc --noEmit` 零 error
...
**目标**：3 个 L2 新增 API endpoint + 1 个推荐规则模块
...
- [ ] **Step 12.2: `POST /api/books/[bookId]/modules/[moduleId]/reset-and-start`**
  ...
  - 行为：
    ...
    2. `UPDATE modules SET learning_status = 'not_started' WHERE id = $1`
    3. 返回 `{ ok: true, redirectUrl: '/modules/{moduleId}/activate' }`
  - **不删除** qa_sessions / test_sessions 历史数据（spec §5.3.5 明确）

- [ ] **Step 12.3: `POST /api/modules/[moduleId]/start-qa`**
  - 请求体：`{}`
  - 行为：
    1. 校验 `learning_status === 'taught'`，否则 400
    2. `UPDATE modules SET learning_status = 'qa_in_progress' WHERE id = $1`
    3. 创建 qa_session（或复用现有 QA 创建逻辑）
    4. 返回 `{ qaSessionId }`
...
- [ ] **Step 12.5: 验证脚本 `scripts/test-m4-task12-l2-apis.ts`**
  - smoke test 三个端点 + book-meta-analyzer 规则
...
- [ ] **Step 18.2: 对话交互**
  - 发消息：`POST /api/teaching-sessions/[sessionId]/messages` with `{ userMessage }`
  - 接收 AI 响应（可能 streaming 或一次返回，取决于 L1 Task 10 实现）
  - 解析响应 `{ status, kpTakeaway, message }`：
    - `status === 'continue'` → 追加 AI 消息，等待用户下一轮
    - `status === 'ready_to_advance'` → 追加 kpTakeaway 总结 → 自动加载下一个 KP cluster
    - `allClustersDone === true` → 跳 `/modules/[moduleId]/teaching-complete`
