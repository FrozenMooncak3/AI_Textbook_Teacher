...
> **云化 patch（2026-04-19 round 3）**：本 plan 写于 2026-04-15 时假设本地 Docker，阶段 1/2 已上线后迁移到 Neon `m4-dev` 分支 + `npm run dev`。经三轮 agent review 修订：
>
...
```bash
# 推荐 C：纯 Node，无需额外安装
node --env-file=.env.local -e "const {Pool}=require('pg');const p=new Pool();p.query('SELECT version()').then(r=>console.log(r.rows[0].version)).finally(()=>p.end())"
```

Expected: 打印 PostgreSQL 16 版本串（Neon 底层 PG16）。
...
```bash
node --env-file=.env.local --experimental-strip-types scripts/test-prompt-templates.ts
```
...
```bash
# 1. Neon m4-dev 分支（开发）
node --env-file=.env.local -e "const {Pool}=require('pg');const p=new Pool();p.query('SELECT COUNT(*)::int AS count FROM knowledge_points').then(r=>console.log(JSON.stringify(r.rows[0]))).catch(e=>{console.error(e);process.exit(1)}).finally(()=>p.end())"
```
...
node --env-file=.env.local --experimental-strip-types scripts/init-neon-schema.ts
...
```bash
node --env-file=.env.local -e "const {Pool}=require('pg');const p=new Pool();p.query(\"INSERT INTO knowledge_points (module_id, kp_code, section_name, description, importance, type, detailed_content) VALUES (1, 'T1', 'test-section', 'test-desc', 3, 'position', 'test-content') RETURNING id\").then(r=>{console.error('UNEXPECTED INSERT SUCCESS',r.rows);process.exit(1)}).catch(e=>console.log('EXPECTED FAIL:',e.message)).finally(()=>p.end())"
```
...
```bash
node --env-file=.env.local -e "const {Pool}=require('pg');const p=new Pool();p.query(\"SELECT column_name FROM information_schema.columns WHERE table_name='knowledge_points' AND column_name='source_anchor'\").then(r=>console.log('rows:',r.rows.length,JSON.stringify(r.rows))).catch(e=>{console.error(e);process.exit(1)}).finally(()=>p.end())"
```
...
```bash
node --env-file=.env.local --experimental-strip-types scripts/test-m4-task1-kp-migration.ts
```
...
```bash
node --env-file=.env.local --experimental-strip-types scripts/test-m4-task3-schema.ts
```
...
```bash
node --env-file=.env.local --experimental-strip-types scripts/test-m4-task4-retry.ts
```
...
```bash
node --env-file=.env.local -e "const {Pool}=require('pg');const p=new Pool();p.query(\"SELECT role, stage, model FROM prompt_templates WHERE role IN ('extractor','coach','examiner','reviewer','assistant') ORDER BY role, stage\").then(r=>console.log(JSON.stringify(r.rows,null,2))).catch(e=>{console.error(e);process.exit(1)}).finally(()=>p.end())"
```
...
```bash
node --env-file=.env.local -e "const {Pool}=require('pg');const p=new Pool();p.query(\"SELECT stage, LENGTH(template_text) AS len, model FROM prompt_templates WHERE role='teacher' ORDER BY stage\").then(r=>console.log('rows:',r.rows.length,JSON.stringify(r.rows,null,2))).catch(e=>{console.error(e);process.exit(1)}).finally(()=>p.end())"
```
...
node --env-file=.env.local -e "const {Pool}=require('pg');const p=new Pool();p.query(\"SELECT m.id AS module_id, c.id AS cluster_id FROM modules m JOIN clusters c ON c.module_id=m.id LIMIT 1\").then(r=>console.log(JSON.stringify(r.rows[0]))).catch(e=>{console.error(e);process.exit(1)}).finally(()=>p.end())"
...
TEST_AUTH_COOKIE="session_token=<cookie>" TEST_MODULE_ID=<id> TEST_CLUSTER_ID=<id> \
  node --env-file=.env.local --experimental-strip-types scripts/test-m4-task10-messages-api.ts
```
...
- [ ] **Step 12.5: 验证脚本 `scripts/test-m4-task12-l2-apis.ts`**
  ...
  ```bash
  node --env-file=.env.local --experimental-strip-types scripts/test-m4-task12-l2-apis.ts
  ```
