...
## 2026-04-20 | M4 Task 14: BookTOC Component (Amber Companion)

**完成内容**: 为项目组件库新增了课程目录（BookTOC）组件，支持普通模式和引导模式。
- **src/components/BookTOC/index.tsx**: 
  - 实现了支持分组展示（moduleGroup）的目录结构。
  - 支持侧边栏收起/展开逻辑（collapsed 状态），并集成 Ctrl+B 快捷键。
  - 提供了引导模式（guideMode）横幅及模块锁定/高亮推荐逻辑。
- **src/components/BookTOC/BookTOCItem.tsx**: 
  - 基于学习状态（learningStatus）动态渲染状态圆点颜色和状态 Badge。
  - 实现了推荐模块的脉冲动画（animate-pulse）及锁定模块的半透明效果。
  - 适配 Amber Companion 悬停效果及圆角规范。
- **验证**: 
  - 通过 npx tsc --noEmit 类型检查。
  - 通过 npm run build 构建测试。
  - 已经在 BookPage (page.tsx) 中注入测试数据，验证了分组展示、Ctrl+B 切换、引导模式下的锁定/推荐/点击逻辑，验证后已恢复代码。

**修改文件**: 
- 新增: src/components/BookTOC/index.tsx, src/components/BookTOC/BookTOCItem.tsx
- 修改: docs/changelog.md
