...
- **book 10**：_待补充当时症状（confirm 时间 / 卡多久 / 具体错误）_
- **book 11**：14MB 教材，`fetch ${OCR_SERVER_URL}/classify-pdf` hang 4-6 分钟后报 `TypeError: fetch failed`。OCR 服务本身正常（外部 curl 4.5s 返 200），瓶颈在 Vercel 函数出站方向
- **book 12**：同 book 11，二次验证一致
