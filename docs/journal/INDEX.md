...
## open（需要关注）
- [strategic:ocr-cost] 🚨 **OCR + KP 提取成本冲击——必须重设成本架构**（2026-04-25）：M4.6 T17 修好后 book 18 实测管线全跑通，但 KP 提取触发 Google AI Studio "credits depleted"。估算单本 8-15 元（369 页），抖音引流 100 人 → 一晚烧 1000 元，账户见底全站宕。下一 session 必须 brainstorm OCR 选型 + KP 模型 + 用户侧防御 + 商业模式。**入口**：读 `docs/superpowers/specs/2026-04-25-ocr-cost-brainstorm-prompt.md` 直接进流程 `[OCR, cost, Gemini, KP-extraction, scaling, Douyin, business-model, paywall, M4.6-closeout]` → [journal](./2026-04-25-ocr-cost-shock.md) · [brainstorm 入口](../superpowers/specs/2026-04-25-ocr-cost-brainstorm-prompt.md)
- [bug:m4.6-t15-oom] ✅ **M4.6 T15+T16+T17 已全部修完且实测通过**：T15 Cloud Run OOM 修（`e0bb0c5`）+ T16 Cloud Build deploy step 自动化（`f097994`）+ T17 Vercel `after()` 包裹 fire-and-forget（`d33a79f`）。book 18 真机实测：28s classify + 6 min OCR 全跑通；下游 KP 阶段失败因 Google AI Studio 余额耗尽（不是代码 bug，是成本问题，转上面 strategic 条目处理）`[M4.6, T15, T16, T17, Cloud-Run, OOM, after, Vercel-isolate, book-18, 369-pages]` → [诊断 journal](./2026-04-22-m4.6-incomplete-fix-diagnosis.md)
...
### 基础设施
- **T1** 🚨 **MVP 上线前 admin 后台监控 UI**（2026-05-02 决策；决策 3-7 brainstorm ⏸️ paused）——决策 3+4 已锁（cookie session + layout 守卫 + `/admin/*` 公开命名），决策 5-7 待 brainstorm（admin UI 信息架构 / 邀请码生成机制 / ban 实现）。**触发条件**：MVP 基础功能完成、用户开始评估上线时刻 resume brainstorm 5-7 → writing-plans → Codex/Gemini 实施。**前置已就绪**：角色字段 + 三层 bypass + entitlement 函数 2026-05-03 已落地（commits 5a8d5da/ee76b54/3fe14a8/5a457b0/a4e15e7），admin UI 直接接进现成骨架即可 `[admin-panel, monitoring, decisions-5-7-paused, MVP-launch, T1, role-base-resolved]` → [WIP](../superpowers/specs/2026-05-02-role-admin-staging-brainstorm-state.md) · [spec](../superpowers/specs/2026-05-02-role-admin-staging-design.md)
...
- [milestone:resolved] **角色系统骨架（决策 1+2）** 落地完成（2026-05-03）：`users.role` enum + `canBypassUploadLimits` entitlement 函数 + presign/confirm 三层 bypass。schema 已应用到生产 Neon main 分支，user_id=1（`frozenmooncak3@gmail.com`）已 seed 为 admin。15 单元测试覆盖（5 presign + 9 confirm + 2 entitlements）双向证明 admin bypass 短路 + regular user 仍命中 D7 拦截。决策 3+4 已锁但未落地（无 UI 就无路径要建）；决策 5-7（admin 监控 UI / 邀请码生成 / ban）暂停到 MVP 上线前 brainstorm resume `[role-system, dev-bypass, entitlement, schema-migration, T1, decisions-1-2-resolved]` → [spec](../superpowers/specs/2026-05-02-role-admin-staging-design.md) · [plan](../superpowers/plans/2026-05-02-role-base.md)
- [milestone:resolved] **M4.7 OCR + KP 成本架构** 正式关闭（2026-05-02）：7 决策全落地（D0 文件类型限制 / D5 DeepSeek 替代 Gemini 10× 成本压缩 / D6 PDF MD5 缓存 / D1 月度预算双层 1.5+500 元 / D7 上传流控）抵御抖音引流烧钱风险。代码 + API smoke + UI 视觉验证（拒绝弹窗 / 进度页 / quota 拦截）全过。M4.6 OCR 管线修复（T15+T16+T17）并入 M4.7 收尾。后续 spillover：T1 Cloud Build trigger 独立 spec + 角色系统加进停车场 T1 `[M4.7, OCR-cost, DeepSeek, MD5-cache, monthly-budget, quota, MVP-protection]` → [spec](../superpowers/specs/2026-04-25-ocr-cost-architecture-design.md) · [plan](../superpowers/plans/2026-04-25-ocr-cost-architecture.md)
